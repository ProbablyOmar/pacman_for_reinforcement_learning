...
for episode in range(EPISODES):
    ...
    while not done:
        ...
        if not done:
            ...
            if np.random.random() > EPSILON:
                ...
                for direction in range(4):
                    if new_observation[direction] == 0 :    #there is no wall in this direction
                        if q_table[tensor_to_tuple(observation)][direction] >= max_next_q_value:
                            max_next_q_value = q_table[tensor_to_tuple(observation)][direction]
            ...
            future_q = q_table[tensor_to_tuple(observation)][next_action]

            current_q = q_table[tensor_to_tuple(observation)][action]
            ...
            q_table[tensor_to_tuple(observation)][action]  = 0
