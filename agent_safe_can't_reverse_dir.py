    ...
    def run(self, is_training=True, render=False):
        ...
        for episode in itertools.count():
            ...
            while(not terminated and episode_reward < self.stop_on_reward):  ## episode starts
                ...
                if is_training and random.random() < epsilon:  ## exploration (select random action or cheat) 
                    ...
                    if np.random.random() > self.cheat_prob and pref_dir != STOP:
                        ...
                        for direction in range(4):
                            if cheat_obs[direction] == 0 and get_direction_value(direction) != -prev_action:  # there is no wall in this direction
                ...
                else:            ### exploitation # select best action  (in this code we always exploit)
                    with torch.no_grad(): 
                        ...
                        if action.item() == -prev_action and looping_count >= 2:
                            ...
                            print(f"number {looping_count} to loop and new direction prefered chosen is {action.item()}")
                ...
                if action.item() == -prev_action:
                    if looping_count %2 == 0:
                        reward += -1
                        print(f"number {looping_count} to loop and got penalized -1")
                    else:
                        print(f"number {looping_count} to loop and not penalized")
                    looping_count += 1
                else:
                    looping_count = 0
